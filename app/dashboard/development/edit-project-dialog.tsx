...
export function EditProjectDialog({ open, onOpenChange, onEditProject, project }: EditProjectDialogProps) {
  ...
  useEffect(() => {
    if (project) {
      ...
      setStatus(project.status || "Pre-construction")
      setWebsite(project.website || "")
      setAmenities(project.amenities?.join(", ") || "")

      // Parse completion date if it exists
      if (project.completionDate) {
        try {
          const date = parse(project.completionDate, "yyyy-MM-dd", new Date())
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          ...
          <DialogDescription>Update the details for this real estate project.</DialogDescription>
        </DialogHeader>
